...
/**
 * Generate a scenario for a given city
 * @param {object} city - City object { name, country, lat, lng }
 * @param {object} emotionProfile - Current emotional profile of the city (or null)
...
async function generateScenario(city, emotionProfile = null) {
    ...
    try {
        ...
        if (!isValidScenario(parsed)) {
            console.warn('[Scenario] Invalid AI response structure, using fallback');
            return getFallbackScenario(city);
        }
...
/**
 * Normalize emotion values to ensure they sum properly and are bounded
 */
